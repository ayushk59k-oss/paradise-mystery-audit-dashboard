...
function lockDashboard(){
  ...
  document.getElementById('signInGate').style.display = '';
...
/* ---------- View tabs ---------- */
const TAB_DEFS = [
  ['central','Central'],['region','City'],['rm','Zonal manager'],
...
function renderStoreBadges(list){
  ...
  list.forEach(r => {
    ...
    const row = document.createElement('div');
    row.className = 'report-row';
    row.innerHTML = '<span>' + r.code + ' &middot; ' + r.name + '</span>' +
...
/* ---------- Separate AM / RM / City boxes — always from full dataset ---------- */
let amChartInstance, rmChartInstance, cityChartInstance;
...
function renderManagerCharts(){
  const amG = groupAvg(r=>r.am), rmG = groupAvg(r=>r.rm), cityG = groupAvg(r=>r.region);
  ...
  if(rmG.labels.length){
    rmChartInstance = new Chart(document.getElementById('rmChart'), {
      type:'bar', data:{labels:rmG.labels, datasets:[{data:rmG.data, backgroundColor:'#eda100', borderRadius:4, maxBarThickness:26}]},
      options:{indexAxis:'y', responsive:true, maintainAspectRatio:false, plugins:{legend:{display:false}}, scales:{x:{min:0,max:110,ticks:{callback:v=>v+'%'}}}},
      plugins:[valueLabelPlugin]
    });
  }
  if(cityG.labels.length){
    cityChartInstance = new Chart(document.getElementById('cityChart'), {
      type:'bar', data:{labels:cityG.labels, datasets:[{data:cityG.data, backgroundColor:'#6250d6', borderRadius:4, maxBarThickness:26}]},
      options:{indexAxis:'y', responsive:true, maintainAspectRatio:false, plugins:{legend:{display:false}}, scales:{x:{min:0,max:110,ticks:{callback:v=>v+'%'}}}},
      plugins:[valueLabelPlugin]
    });
  }
...
document.getElementById('gateSignInBtn').addEventListener('click', () => {
  ...
  msg.textContent = 'Opening Google sign-in…'; msg.className = 'status-msg';
  requestAccessToken().catch(e => {
    msg.textContent = 'Sign-in failed: ' + e.message; msg.className = 'status-msg err';
  });
});
...
(async function init(){
  ...
  renderEntityPicker();
  renderReportList();
  // Dashboard content stays hidden behind the sign-in gate until unlockDashboard() runs.
})();
