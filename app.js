...
async function loadLocal(){
  ...
  if(publicData){
    try{
      ...
      await saveLocal(false); // cache it locally too, as a fallback for next time
      return;
...
function renderServing(list){
  ...
  el.innerHTML = '';
  list.forEach(r => (r.serving||[]).forEach(([p,m]) => {
...
function renderCityAndZonalScores(){
  ...
  function renderList(elId, entries, emptyMsg){
  ...
  renderList('cityScoreList', avgByKey(r => r.region), 'No reports yet.');
  renderList('zonalScoreList', avgByKey(r => r.rm), 'No reports yet.');
}
